...
#import "SRKResizeableCropOverlayView.h"

#import <QuartzCore/QuartzCore.h>

#define rad(angle) ((angle) / 180.0 * M_PI)

static CGRect SRKScaleRect(CGRect rect, CGFloat scale)
{
...
@interface SRKImageCropView ()<UIScrollViewDelegate>
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) UIImageView *imageView;
@property (nonatomic, strong) SRKImageCropOverlayView *cropOverlayView;
...
@implementation SRKImageCropView
...
- (void)setCropSize:(CGSize)cropSize{
    ...
    if (self.cropOverlayView == nil){
        if(self.resizableCropArea)
            self.cropOverlayView = [[SRKResizeableCropOverlayView alloc] initWithFrame:self.bounds andInitialContentSize:CGSizeMake(cropSize.width, cropSize.height)];
        else
            self.cropOverlayView = [[SRKImageCropOverlayView alloc] initWithFrame:self.bounds];
...
- (CGRect)_calcVisibleRectForResizeableCropArea{
    SRKResizeableCropOverlayView* resizeableView = (SRKResizeableCropOverlayView*)self.cropOverlayView;
    ...
    return visibleRect = SRKScaleRect(visibleRect, sizeScale);
}
...
-(CGRect)_calcVisibleRectForCropArea{
    ...
    return visibleRect = SRKScaleRect(visibleRect, scale);
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    SRKResizeableCropOverlayView* resizeableCropView = (SRKResizeableCropOverlayView*)self.cropOverlayView;
