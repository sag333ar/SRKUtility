//
//  SRKResizeableCropOverlayView.m
//  SRKImagePicker
//
...
@interface SRKResizeableCropOverlayView(){
    CGSize _initialContentSize;
    BOOL _resizingEnabled;
    CGPoint _theAnchor;
    CGPoint _startPoint;
    SRKResizeableViewBorderMultiplyer _resizeMultiplyer;
}
...
@implementation SRKResizeableCropOverlayView
...
-(void)_addContentViews{
   ...
   // NSLog(@"x: %f y: %f %f", CGRectGetMinX(_contentView.frame), CGRectGetMinY(_contentView.frame), self.bounds.size.width);
    ...
    _cropBorderView = [[SRKCropBorderView alloc] initWithFrame:CGRectMake(self.bounds.size.width / 2 - _initialContentSize.width  / 2 - kBorderCorrectionValue, (self.bounds.size.height - toolbarSize) / 2 - _initialContentSize.height / 2 - kBorderCorrectionValue, _initialContentSize.width + kBorderCorrectionValue*2, _initialContentSize.height + kBorderCorrectionValue*2)];
